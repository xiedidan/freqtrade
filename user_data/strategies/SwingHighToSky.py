...
class SwingHighToSky(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for val in self.buy_cciTime.range:
            new_columns[f'cci-{val}'] = ta.CCI(dataframe, timeperiod=val)

        for val in self.sell_cciTime.range:
            new_columns[f'cci-sell-{val}'] = ta.CCI(dataframe, timeperiod=val)

        for val in self.buy_rsiTime.range:
            new_columns[f'rsi-{val}'] = ta.RSI(dataframe, timeperiod=val)

        for val in self.sell_rsiTime.range:
            new_columns[f'rsi-sell-{val}'] = ta.RSI(dataframe, timeperiod=val)

        # 创建包含新列的 DataFrame
        new_columns_df = DataFrame(new_columns, index=dataframe.index)
